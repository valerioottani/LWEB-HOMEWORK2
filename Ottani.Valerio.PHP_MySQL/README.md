...
1. Creare il database denominato `valerio.ottani.PHP-MySQL`,`giovanni.lapenna.PHP-MySql`.
2. Generare le tabelle relazionali necessarie (`artisti`, `utenti`, `album`, `tracce`, `eventi`, `posti_evento`) complete di vincoli (FOREIGN KEY e ON DELETE CASCADE).
...
4. eventi.php e prenota_evento.php: Calendario dei concerti live e pagina di prenotazione dei biglietti. La sezione di prenotazione integra:
   - **Card Visive Interattive con Immagini Dedicate:** Selezione del settore supportata da anteprime grafiche (`standing.png` per il Parterre, `superiore.png` per la Tribuna VIP e `anello.png` per il Primo Anello).
   - **Filtro Dinamico in JavaScript:** Filtraggio istantaneo della disponibilità dei posti in base alla card del settore cliccata.
   - **Gestione Real-time dei Posti:** Aggiornamento dello stato del posto nel database al momento dell'acquisto ed inserimento nel carrello di sessione.
